...
struct sampler {
    ...
    sampler_callback_t     on_captured;
};

static void* sampler_thread_fn(void* sampler_ptr)
{
    sampler_t* sampler = sampler_ptr;
    ...
    while (1) {
        ...
        ctm_sleep_ms(sampler->interval_ms);
    }
    return NULL;
}

sampler_t* sampler_create(const unsigned int           interval_ms,
                          const size_t                 item_size,
                          const sampler_capture_func_t capture_fn)
{
    sampler_t* sampler;
    ...
    sampler = malloc(sizeof(sampler_t));
    if (sampler == NULL) {
...
int sampler_subscribe(sampler_t*               sampler,
                      const sampler_callback_t subscriber)
{
...
long sampler_get_value(sampler_t*                   sampler,
                       const size_t                 window,
                       const sampler_process_func_t process_fn)
{
...
sampler_t* sampler_create_with_subscription_and_start(const unsigned int           interval_ms,
                                                      const size_t                 item_size,
                                                      const sampler_capture_func_t capture_fn,
                                                      const sampler_callback_t     callback)
{
    sampler_t* sampler;
