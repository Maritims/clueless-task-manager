...
struct CtmAudioStreamSource {
    ...
    int                    running;                /* Application-level state of the playback stream. */
    ...
    int              file_eof;         /* This flag indicates whether the end of the input sound file has been reached. */
...
static void on_playback_process(void* userdata)
{
    ...
    uint32_t                 required_bytes, index, readable_bytes, bytes_to_copy;
    ...
    if (stream_buffer_handle == NULL) {
        fprintf(stderr, "on_playback_process: stream: Failed to dequeue buffer: %s\n", strerror(errno));
        return;
    }

    raw_audio_buffer       = stream_buffer_handle->buffer;
    outbound_audio_payload = raw_audio_buffer->datas[0].data; /* This is the water which we're looking for in the bucket. */
    if (outbound_audio_payload == NULL) {
        fprintf(stderr, "on_playback_process: stream: Failed to get outbound audio payload\n");
        return;
    }

    bytes_per_audio_frame = sizeof(int16_t) * audio_stream_source->input_snd_file_info.channels; /* Our "step length", also known as "stride". */
    required_bytes        = raw_audio_buffer->datas[0].maxsize;
    readable_bytes        = spa_ringbuffer_get_read_index(&audio_stream_source->ring, &index);
    bytes_to_copy         = readable_bytes < required_bytes ? readable_bytes : required_bytes;
                                           ...
                                           void*            user_data)
{
    ...
    audio_stream_source = malloc(sizeof(CtmAudioStreamSource));
    if (audio_stream_source == NULL) {
        fprintf(stderr, "ctm_sound_play_async: Failed to allocate memory\n");
        return NULL;
    }
    ...
    if (!audio_stream_source->input_snd_file_handle) {
        fprintf(stderr, "ctm_sound_play_async: Failed to open file %s: %s\n", file, sf_strerror(NULL));
