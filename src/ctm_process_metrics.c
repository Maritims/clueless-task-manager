...
static int get_pid_file_path(const unsigned int pid, char* out_buffer) {
    const int bytes_written = snprintf(out_buffer, BUFFER_SIZE, "/proc/%u/stat", pid);
...
static int get_user_from_uid(const uid_t uid, char* out_buffer) {
    int            bytes_written;
    struct passwd  pwd;
    ...
    bytes_written = snprintf(out_buffer, BUFFER_SIZE, "%s", pwd.pw_name);
...
ctm_process_metrics_status_t process_get_pid(const ctm_process_metrics_t* process_metrics, unsigned long* out_value) {
...
ctm_process_metrics_status_t process_get_name(const ctm_process_metrics_t* process_metrics, char* out_buffer, size_t out_buffer_size) {
    if (process_metrics == NULL || out_buffer == NULL || out_buffer_size == 0) {
        return CTM_PROCESS_METRICS_ERR_INVALID_ARG;
    }

    snprintf(out_buffer, out_buffer_size, "%s", process_metrics->name);
    return CTM_PROCESS_METRICS_SUCCESS;
}

ctm_process_metrics_status_t process_get_state(const ctm_process_metrics_t* process_metrics, char* out_buffer, size_t out_buffer_size) {
    ...
    switch (process_metrics->state) {
        case 'R':
            snprintf(out_buffer, out_buffer_size, "Running");
            break;
        case 'S':
            snprintf(out_buffer, out_buffer_size, "Sleeping");
            break;
        case 'D':
            snprintf(out_buffer, out_buffer_size, "Disk Sleep");
            break;
        case 'Z':
            snprintf(out_buffer, out_buffer_size, "Zombie");
            break;
        case 'T':
            snprintf(out_buffer, out_buffer_size, "Traced/Stopped");
            break;
        case 'W':
            snprintf(out_buffer, out_buffer_size, "Paging/Waking");
            break;
        case 'X':
        case 'x':
            snprintf(out_buffer, out_buffer_size, "Dead");
            break;
        case 'K':
            snprintf(out_buffer, out_buffer_size, "Wake-kill");
            break;
        case 'P':
            snprintf(out_buffer, out_buffer_size, "Parked");
            break;
        case 'I':
            snprintf(out_buffer, out_buffer_size, "Idle");
            break;
        default:
            return CTM_PROCESS_METRICS_ERR_PARSE;
    }
...
ctm_process_metrics_status_t process_get_user(const ctm_process_metrics_t* process_metrics, char* out_buffer, size_t out_buffer_size) {
    if (process_metrics == NULL || out_buffer == NULL || out_buffer_size == 0) {
        return CTM_PROCESS_METRICS_ERR_INVALID_ARG;
    }

    snprintf(out_buffer, out_buffer_size, "%s", process_metrics->user);
    return CTM_PROCESS_METRICS_SUCCESS;
}

ctm_process_metrics_status_t process_get_is_active(const ctm_process_metrics_t* process_metrics, unsigned int* out_value) {
    if (process_metrics == NULL || out_value == NULL) {
        return CTM_PROCESS_METRICS_ERR_INVALID_ARG;
    }
...
ctm_process_metrics_status_t process_set_is_active(ctm_process_metrics_t* process_metrics, const int is_active) {
