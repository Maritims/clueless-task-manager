...
struct CtmProcess {
    ...
    unsigned long long total_time; // calculated
};
...
CtmProcess* CtmProcess_LoadFromKernel(const pid_t pid, const int include_all_users) {
    char        path[64];
    ...
    return CtmProcess_Create(pid, uid, name, state, username, utime, stime, rss_kb, starttime);
}

CtmProcessArray* CtmProcessArray_LoadFromKernel(const int include_all_users) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        const CtmProcess* process = CtmProcess_LoadFromKernel(pid, include_all_users);
        if (process == NULL) {
            continue;
        }

        const int push_result = CtmProcessArray_Push(array, process);
        if (push_result != 0) {
...
const CtmProcess* CtmProcessArray_GetElement(const CtmProcessArray* array, const size_t index) {
    if (array == NULL) {
...
unsigned long long CtmProcess_GetCPUUsageScaled(const CtmProcess* process) {
    if (process == NULL) {
        fprintf(stderr, "ctm_process_get_usage_scaled: process cannot be NULL\n");
        return 0;
    }

    size_t file_size     = 0;
    char*  uptime_buffer = cio_read_file("/proc/uptime", &file_size);
    if (uptime_buffer == NULL) {
        fprintf(stderr, "%s: Failed to read /proc/uptime: %s\n", __func__, strerror(errno));
        return 0;
    }
    const long double system_uptime_seconds = strtod(uptime_buffer, NULL);
    free(uptime_buffer);
    const long double        system_uptime_ticks   = system_uptime_seconds * sysconf(_SC_CLK_TCK);
    const long double        process_elapsed_ticks = system_uptime_ticks - process->start_time;
    const unsigned long long usage_scaled          = 10000ULL * ((process->utime + process->stime) / process_elapsed_ticks);
...
int CtmProcessArray_Push(CtmProcessArray* handle, const CtmProcess* item) {
    ...
    if (handle->count >= handle->capacity) {
        const size_t new_capacity = handle->capacity == 0 ? 100 : handle->capacity * 2;
        CtmProcess*     new_ptr      = realloc(handle->elements, new_capacity * sizeof(CtmProcess));
        if (!new_ptr) {
            fprintf(stderr, "Failed to allocate memory for process array: %d\n", errno);
            return errno;
        }
        handle->elements = new_ptr;
        handle->capacity = new_capacity;
    }

    handle->elements[handle->count++] = *item;
