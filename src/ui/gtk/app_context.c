...
AppContext* app_context_create(void)
{
    ...
    if ((ctx->cpu_sampler = sampler_create_with_subscription_and_start(500, cpu_size(), (SamplerCaptureFunc) cpu_capture, (SamplerCallback) on_cpu_sampled)) == NULL) {
...
void app_context_update(AppContext* ctx)
{
    ...
    ctx->total_cpu_usage    = sampler_get_value(ctx->cpu_sampler, 10, (SamplerProcessFunc) cpu_get_total_usage);
